...
* The application retains its "Self-Healing" category logic. Upon the first successful Google login, the app checks for existing categories. If none are found, it automatically seeds the 8 standard categories (*Work, Groceries, Medical, etc.*) into the new user's specific Firestore path.
...
*Note: This is often triggered automatically by enabling Google Auth.*
