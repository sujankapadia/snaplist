      ...
      <main className="max-w-2xl mx-auto p-4 space-y-3">
        ...
        {filteredAndSortedTasks.map((task) => (
          <div
            ...
            className={`group flex items-start gap-3 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all active:scale-[0.98] ${
          ...
          >
            <button
              onClick={(e) => {
                e.stopPropagation();
                updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', task.id), {
            ...
            <div className="p-6 space-y-6 overflow-y-auto no-scrollbar">
              <div className="space-y-1">
                ...
                <input
                  ...
                  onBlur={() =>
                    updateDoc(
                      doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', editingTask.id),
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateDoc(
                        doc(db, 'artifacts', appId, 'users', user.uid, 'tasks', editingTask.id),
                        { category: val, isNewCategory: false }
                      );
                    }}
                  >
                    {categories.map((c) => (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-black text-slate-400 uppercase tracking-widest text-xs text-center">
                Sort Tasks By
              </h3>
            ...
            <div className="p-4 overflow-y-auto space-y-4 no-scrollbar">
              ...
              <div className="space-y-3">
                {categories.map((cat) => (
                  <div
                    key={cat.id}
                    className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border dark:border-slate-800"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span
                      ...
                      <button
                        onClick={() =>
                          deleteDoc(
                            doc(db, 'artifacts', appId, 'users', user.uid, 'categories', cat.id)
                          )
                        }
