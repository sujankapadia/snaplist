...
export default function App() {
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortMethod === 'urgency') return (URGENCY_LEVELS[b.urgency] || 0) - (URGENCY_LEVELS[a.urgency] || 0);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-24 font-sans transition-colors duration-200">
      <header className="sticky top-0 z-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-100">SnapList</h1>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setShowCategoryManager(true)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-500 transition-colors"><Settings size={20}/></button>
          <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-500 transition-colors">{isDarkMode ? <Sun size={20} /> : <Moon size={20} />}</button>
        </div>
      </header>
      ...
      <div className="sticky top-[57px] z-10 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800 p-2 space-y-2">
        <div className="flex items-center gap-2 max-w-2xl mx-auto px-2">
          ...
          <button onClick={() => setShowSortMenu(true)} className="flex items-center gap-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-xl text-xs font-bold shadow-sm active:scale-95 transition-transform"><ArrowUpDown size={14} /> Sort</button>
        </div>
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1 px-4 max-w-2xl mx-auto">
          <button onClick={() => setStatusFilter(statusFilter === 'active' ? 'completed' : 'active')} className={`flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border transition-all ${statusFilter === 'completed' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-slate-100 text-slate-500 border-slate-200 dark:bg-slate-800 dark:border-slate-700'}`}>{statusFilter === 'completed' ? 'Done ✓' : 'Active'}</button>
          <div className="h-4 w-px bg-slate-300 dark:bg-slate-700 flex-shrink-0 mx-1" />
          {['all', 'today', 'week', 'month'].map(f => (
            <button key={f} onClick={() => setDateFilter(f)} className={`flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border transition-all ${dateFilter === f ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800'}`}>{f}</button>
          ))}
          <div className="h-4 w-px bg-slate-300 dark:bg-slate-700 flex-shrink-0 mx-1" />
          <button onClick={() => setCategoryFilter(null)} className={`flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase border ${!categoryFilter ? 'bg-slate-800 text-white' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800'}`}>All</button>
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setCategoryFilter(cat.name)} style={categoryFilter === cat.name ? getCategoryStyles(cat.name) : {}} className={`flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase border transition-all ${categoryFilter === cat.name ? 'shadow-md scale-105' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800'}`}>{cat.name}</button>
          ))}
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <span style={getCategoryStyles(task.category)} className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full">{task.category}</span>
                {task.dueDate && (
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-slate-50 dark:from-slate-950 to-transparent pointer-events-none">
        <div className="max-w-2xl mx-auto flex items-center gap-3 pointer-events-auto">
          ...
          <button onClick={toggleListening} className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl text-white active:scale-90 transition-all ${isListening ? 'bg-red-500 animate-pulse' : 'bg-indigo-600 shadow-indigo-500/30'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in slide-in-from-bottom duration-300">
            <div className="p-4 border-b dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
              ...
              <button onClick={() => setEditingTask(null)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors"><X size={20}/></button>
            </div>
            ...
            <div className="p-6 space-y-6 overflow-y-auto no-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Urgency</label>
                  <select 
                    className="w-full bg-slate-100 dark:bg-slate-800 p-2.5 rounded-xl text-sm font-semibold outline-none ring-indigo-500/20 focus:ring-2 text-slate-800 dark:text-white"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1"><Calendar size={12}/> Due Date</label>
                <input 
                  type="datetime-local"
                  className="w-full bg-slate-100 dark:bg-slate-800 p-2.5 rounded-xl text-sm font-semibold outline-none ring-indigo-500/20 focus:ring-2 text-slate-800 dark:text-white"
              ...
              <div className="space-y-3">
                ...
                {uploadingFile && (
                  <div className="flex items-center gap-3 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-dashed border-indigo-200 dark:border-indigo-800 animate-pulse">
                    ...
                    <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 tracking-wide">Uploading file...</span>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-3">
                  {editingTask.attachments?.map((file) => (
                    <div key={file.id} className="group relative bg-slate-50 dark:bg-slate-800 rounded-xl border dark:border-slate-700 overflow-hidden shadow-sm">
                      ...
                      <div className="p-2 flex items-center justify-between gap-1 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm">
                        ...
                        <div className="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                          <a href={file.url} target="_blank" rel="noopener noreferrer" className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300">
                            <ExternalLink size={12}/>
                          </a>
                          <button onClick={() => removeAttachment(file)} className="p-1 hover:bg-red-50 dark:hover:bg-red-900/30 rounded text-red-500">
                            <Trash2 size={12}/>
                          </button>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="p-4 border-b dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
              ...
              <button onClick={() => setShowCategoryManager(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={20}/></button>
            </div>
            <div className="p-4 overflow-y-auto space-y-4 no-scrollbar">
              ...
              <button onClick={() => addDoc(collection(db, 'users', user.uid, 'categories'), { name: "New Category", description: "", hue: generateCategoryHue(), createdAt: serverTimestamp() })} className="w-full py-3 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl text-slate-400 hover:text-indigo-600 hover:border-indigo-600 font-bold transition-all flex items-center justify-center gap-2 group">
                <Plus size={18} className="group-hover:scale-125 transition-transform" /> Add Category
              </button>
            </div>
            <div className="p-4 border-t dark:border-slate-800 text-center bg-slate-50 dark:bg-slate-900/50">
              <button onClick={() => setShowCategoryManager(false)} className="px-10 py-2.5 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-500/20 active:scale-95 transition-all">Close</button>
            </div>
          </div>
        </div>
      )}
